import {
  asRecord,
  readString as readProtocolString,
...
export const APPROVAL_SCOPE_OPTIONS: ApprovalScopeOption[] = [
  {
    scope: 'single',
    label: 'Once',
    enabled: true,
...
export function approvalScopeForDecision(decision: ApprovalDecision): ApprovalDecisionScope {
  return decision === 'acceptForSession' ? 'session' : 'single'
}

export function approvalDecisionForScope(scope: ApprovalDecisionScope): ApprovalDecision {
...
function maxRisk(first: ApprovalRiskLevel, second: ApprovalRiskLevel): ApprovalRiskLevel {
  const order: Record<ApprovalRiskLevel, number> = { low: 0, medium: 1, high: 2 }
...
function analyzeCommand(command: string, t: ApprovalRiskTranslator): Pick<ApprovalRiskSummary, 'level' | 'riskLabels' | 'impacts'> {
  const normalized = command.trim()
  let level: ApprovalRiskLevel = normalized ? 'low' : 'medium'
...
function buildCommandApprovalSummary(request: ApprovalRequest, t: ApprovalRiskTranslator): ApprovalRiskSummary {
...
function buildFileChangeApprovalSummary(request: ApprovalRequest, t: ApprovalRiskTranslator): ApprovalRiskSummary {
  const params = asRecord(request.params)
  const grantRoot = readString(params?.grantRoot)
  const reason = readString(params?.reason)
  const cwd = readString(params?.cwd)
  let level: ApprovalRiskLevel = grantRoot ? 'medium' : 'low'
...
function buildGenericApprovalSummary(request: ApprovalRequest, t: ApprovalRiskTranslator): ApprovalRiskSummary {
  const params = asRecord(request.params)
  const reason = readString(params?.reason)
