...
function failTurnOutboxMessages(messages, turnId, error) {
    let changed = false;
    const next = messages.map((message) => {
        if (message.role !== 'user' || message.turnId !== turnId || !message.outbox)
            return message;
...
export function reduceConversationEvent(previous, event) {
    ...
    if (event.type === 'turn.disconnected') {
        ...
        return {
            ...updated,
            messages: failTurnOutboxMessages(updated.messages, turnId, eventText(event.data, 'Codex upstream response stream disconnected.')),
    ...
    if (event.type === 'command.failed') {
        ...
        const messages = state.messages.map((message) => message.id === messageId
            ? { ...message, messageType: 'userMessage.outbox.failed', outbox: { status: 'failed', lastError: error } }
            : message);
