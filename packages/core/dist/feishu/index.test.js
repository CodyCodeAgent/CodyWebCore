...
describe('Feishu interactive cards', () => {
    ...
    it('splits long Markdown without dropping content', () => {
        ...
        expect(cards.map(card => (card.body.elements[0]?.content)).join('')).toBe(markdown.replace('\n', ''));
...
describe('Feishu application administration', () => {
    it('fetches and normalizes one message detail for quote expansion', async () => {
        ...
        Object.assign(provider, { client: { im: { v1: { message: { get } } } } });
    ...
    it('attaches exactly one same-conversation quoted message to an inbound envelope', async () => {
        ...
        const enriched = await provider.resolveQuotedMessage(current);
    ...
    it('resolves and caches user-facing chat metadata', async () => {
        ...
        Object.assign(provider, { client: { im: { v1: { chat: { get } } } } });
