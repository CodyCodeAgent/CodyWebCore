...
export function normalizeFeishuMessage(config, payload) {
    ...
    const senderIdentities = [...new Map([
        ...
        ].filter((entry) => Boolean(entry[1])).map(([idType, id]) => [id, { id, idType }])).values()];
...
export function feishuMarkdownCards(markdown, options = {}) {
    ...
    if (current.length || groups.length === 0)
        groups.push(current.length ? current : [{ tag: 'markdown', content: ' ' }]);
    return groups.map((elements, index) => feishuMarkdownCardFromElements(elements, options.note ? (groups.length > 1 ? `${options.note}  |  ${index + 1}/${groups.length}` : options.note) : undefined));
}
...
export function feishuStreamingCard(input) {
    ...
    if (input.note)
        elements.push(feishuV2Note(input.note));
    return {
        schema: '2.0',
...
function feishuMarkdownCardFromElements(elements, note) {
    const bodyElements = elements.length ? [...elements] : [{ tag: 'markdown', content: ' ' }];
    if (note)
        bodyElements.push(feishuV2Note(note));
    return {
        schema: '2.0',
        config: { update_multi: true },
...
function feishuV2Note(note) {
    return { tag: 'markdown', content: feishuCardMarkdown(`---\n${note.slice(0, 500)}`) };
}
function sourceLines(lines, map) {
...
function feishuTableElement(tokens) {
    const headers = [];
    const rows = [];
    let inHeader = false;
    let inBody = false;
...
function feishuMarkdownElements(markdown) {
    if (!markdown.trim())
    ...
    while (index < tokens.length) {
        ...
        if (token.type === 'table_open') {
            ...
            if (table)
                ...
                buffer.push(sourceLines(lines, token.map));
            index = closeIndex + 1;
            continue;
        }
...
function splitOversizedFeishuElement(element) {
    ...
    if (value.tag !== 'table' || !Array.isArray(value.rows) || JSON.stringify(value).length <= FEISHU_CARD_ELEMENT_BUDGET)
        return [value];
