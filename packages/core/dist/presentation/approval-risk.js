import { asRecord, readString as readProtocolString, isCommandApprovalRequestMethod, isFileChangeApprovalRequestMethod, } from '../protocol/index.js';
...
export const APPROVAL_SCOPE_OPTIONS = [
    {
        scope: 'single',
        label: 'Once',
        enabled: true,
...
export function translateApprovalRiskMessage(key, replacements = {}) {
...
const defaultApprovalRiskTranslator = translateApprovalRiskMessage;
export function approvalScopeForDecision(decision) {
    return decision === 'acceptForSession' ? 'session' : 'single';
}
export function approvalDecisionForScope(scope) {
...
function maxRisk(first, second) {
    const order = { low: 0, medium: 1, high: 2 };
    return order[first] >= order[second] ? first : second;
}
function commandContains(command, patterns) {
...
function analyzeCommand(command, t) {
    const normalized = command.trim();
    let level = normalized ? 'low' : 'medium';
    const riskLabels = [];
    const impacts = [];
    if (!normalized) {
...
function isOutsideCwd(pathValue, cwd) {
    if (!pathValue || !cwd)
        return false;
...
function buildCommandApprovalSummary(request, t) {
...
function buildFileChangeApprovalSummary(request, t) {
    const params = asRecord(request.params);
    const grantRoot = readString(params?.grantRoot);
    const reason = readString(params?.reason);
    const cwd = readString(params?.cwd);
    let level = grantRoot ? 'medium' : 'low';
...
function buildGenericApprovalSummary(request, t) {
    const params = asRecord(request.params);
    const reason = readString(params?.reason);
...
export function buildApprovalRiskSummary(request, t = defaultApprovalRiskTranslator) {
...
//# sourceMappingURL=approval-risk.js.map