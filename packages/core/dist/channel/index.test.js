...
import { createConversationState, reduceConversationEvents } from '../conversation/index.js';
import { ReliableChannelOutbox, channelCommandId, channelConversationKey, extractMarkdownImageReferences, projectChannelTurn, stripMarkdownImages } from './index.js';
const message = {
    ...
    it('retries transient failures and dead-letters permanent failures', async () => {
        ...
        const store = {
            async enqueue(input) {
                ...
                if (existing)
                    return existing;
                const row = { ...input, status: 'pending', attempts: 0, availableAtIso: input.availableAtIso ?? now.toISOString() };
                rows.set(row.id, row);
                return row;
            },
            async claim(input) {
        ...
        const queue = new ReliableChannelOutbox({ provider: 'feishu', accountId: 'bot-1' }, store, {
            async deliver(item) { expect(rows.get(item.id)?.status).toBe('sending'); calls += 1; if (calls === 1)
                throw new Error('timeout'); return { remoteMessageId: 'remote-1' }; },
